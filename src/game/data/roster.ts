...
import { gainProficiencyExp } from './promotions';
...
const ROSTER_DEFS: CreateCharacterOptions[] = [
  {
    ...
    weaponMastery: { sword: 3 }, skillLoadout: ['power_strike', 'sword_crescent', 'sword_flash', 'sword_blink'],
  ...
  },
  {
    ...
    weaponMastery: { blunt: 3 }, skillLoadout: ['power_strike', 'blunt_leghit', 'blunt_shove', 'blunt_wideguard'],
  },
  {
    ...
    weaponMastery: { spear: 3 }, skillLoadout: ['power_strike', 'spear_pierce', 'spear_lock', 'spear_charge'],
  },
  {
    ...
    weaponMastery: { bow: 3 }, skillLoadout: ['power_strike', 'bow_skyshot', 'bow_leapshot', 'bow_snipe'],
  ...
  },
  {
    ...
    weaponMastery: { crossbow: 3 }, skillLoadout: ['power_strike', 'xbow_ap', 'xbow_pierceshot', 'xbow_lethal'],
  },
  {
    ...
    weaponMastery: { dagger: 3 }, skillLoadout: ['power_strike', 'dagger_ambush', 'dagger_stealth', 'dagger_warp'],
  },
  {
    ...
    weaponMastery: { thrown: 3 }, skillLoadout: ['power_strike', 'thrown_poison', 'thrown_clone', 'thrown_chain'],
  },
  {
    ...
    weaponMastery: { staff: 3 }, skillLoadout: ['incantation', 'staff_bolt', 'staff_weaken', 'staff_meteor'],
  },
  {
  ...
  },
  {
    ...
    weaponMastery: { tome: 3 }, skillLoadout: ['incantation', 'tome_heal', 'tome_purify', 'tome_recast'],
  },
];

// 세션 동안 유지되는 편집 가능한 캐릭터 인스턴스(인벤토리에서 무기/스킬 등을 여기서 직접 수정).
// 테스트/빠른 전투용 로스터는 전직 3차 + 무기 숙련 달인(피해 하한 100%)으로 둔다.
export const ROSTER: Character[] = ROSTER_DEFS.map((entry) => {
  const c = createCharacter(entry);
  for (const kind of Object.keys(entry.weaponMastery ?? {}) as WeaponKind[]) gainProficiencyExp(c, kind, 60);
  return c;
});
